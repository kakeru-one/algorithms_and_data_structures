...
void saiten(vector<vector<int> > &answer, int &correct_count, int &incorrect_count) {
  for (int i = 0; i < 9; i++) {
    for (int j = 0; j < 9; j++) {
      int correct_cell = (i + 1) * (j + 1);
      // Aのうち、正しい値の書かれたマスの個数を correct_count に入れる
      // Aのうち、誤った値の書かれたマスの個数を wrong_count に入れる
      if (answer.at(i).at(j) == correct_cell) {
        correct_count++;
      } else {
        // 呼び出し側のAの各マスを正しい値に修正する
        answer.at(i).at(j) = correct_cell;
        incorrect_count++;
      }
    }
  }
